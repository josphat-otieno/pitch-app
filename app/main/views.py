...
def index():
    ...
    return render_template('index.html', header=header, pitches = pitches, education = education, love = love, business = business, interview = interview, promotion = promotion)
...
def new_pitch():
    ...
    if form.validate_on_submit():
        ...
        user_id = current_user._get_current_object().id
        new_pitch_object = Pitch(post_pitch=post_pitch,user_id =user_id,category=category,title=title)
